...
contract MasterChef is Ownable {
    ...
    // The EGG TOKEN!
    DBallToken public dball;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public dballPerBlock;
    // Bonus muliplier for early dball makers.
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _dballPerBlock,
        uint256 _startBlock
    ) public {
        dball = _dball;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        dballPerBlock = _dballPerBlock;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, uint16 _unstakingFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 200, "add: invalid deposit fee basis points");
        require(_unstakingFeeBP <= 300, "add: invalid unstaking fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint16 _unstakingFeeBP, bool _withUpdate) public onlyOwner {
        require(_depositFeeBP <= 300, "set: invalid deposit fee basis points");
        require(_unstakingFeeBP <= 200, "set: invalid unstaking fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 dballReward = multiplier.mul(dballPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(dballReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 dballReward = multiplier.mul(dballPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        dball.mint(devaddr, dballReward.div(10));
        dball.mint(address(this), dballReward);
        pool.accEggPerShare = pool.accEggPerShare.add(dballReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe dball transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 dballBal = dball.balanceOf(address(this));
        if (_amount > dballBal) {
            dball.transfer(_to, dballBal);
        } else {
            dball.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _dballPerBlock) public onlyOwner {
        massUpdatePools();
        require(_dballPerBlock>=30000000000000000000, "You cannot make DBALL Per Block more than 30 DBALL");
        dballPerBlock = _dballPerBlock;
    }
}
